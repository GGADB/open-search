...
本指南说明如何在Claude Code中安装和使用开放搜索skill。
...
#### 步骤3：安装Skill
```bash
npx skills add jackwener/opencli --skill open-search
```
...
# 检查skill
# 在Claude Code中输入：/open search 测试
```
...
# Browser Harness配置文件
~/.config/browser-harness/config.json
```

## 常见问题

### 问题1：skill未识别
**症状**：输入`/open search`无反应
...
```bash
# 编辑路由规则
vim ~/.claude/skills/open-search/references/routing-rules.md
```
...
```bash
# 查看skill文件
cat ~/.claude/skills/open-search/SKILL.md

# 查看参考文件
ls ~/.claude/skills/open-search/references/
```
