...
```bash
# 复制skill文件到Codex技能目录
cp -r open-search ~/.codex/skills/
```

**方法2：使用Codex CLI**
```bash
# 如果Codex支持CLI安装
codex skill add open-search
```
...
```bash
# 编辑路由规则
vim ~/.codex/skills/open-search/references/routing-rules.md
```
...
```bash
# 查看skill文件
cat ~/.codex/skills/open-search/SKILL.md

# 查看参考文件
ls ~/.codex/skills/open-search/references/
```
...
### 已知兼容性问题
1. **技能调用语法**：Codex可能使用不同的语法
2. **输出格式**：Codex可能对输出格式有特定要求
3. **错误处理**：Codex可能有特定的错误处理方式

### 解决方案
1. **查阅Codex文档**：了解skill安装和使用规范
2. **测试兼容性**：在Codex中测试所有功能
3. **反馈问题**：向Codex团队反馈兼容性问题
